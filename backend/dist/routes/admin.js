...
import { createMockUser, deleteMockUser, listMockUsers, resetMockDocumentsFromSample, updateMockUser, } from '../mock/store.js';
...
router.get('/users', requireAdmin, async (_req, res) => {
    try {
        ...
        const { rows } = await pool.query(`SELECT u.id, u.email, u.name, u.is_admin, u.created_at, u.updated_at,
...
router.post('/users', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        const { rows } = await pool.query(`INSERT INTO users (email, password_hash, name, is_admin)
       VALUES ($1, $2, $3, $4)
       RETURNING id, email, name, is_admin, created_at, updated_at`, [normalizedEmail, hashPassword(normalizedPassword), normalizedName, parseBoolean(isAdmin)]);
...
router.patch('/users/:id', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        WHERE id = $${paramIndex}
        RETURNING id, email, name, is_admin, created_at, updated_at`, params);
        if (rows.length === 0) {
            return res.status(404).json({ ok: false, error: 'user_not_found' });
        }
...
router.post('/seed', requireAdmin, async (req, res) => {
    ...
    try {
        ...
        for (const doc of docs) {
            try {
                await pool.query(`INSERT INTO documents (id, user_id, filename, original_name, file_path, file_size, file_type, is_public)
           ...
           is_public = EXCLUDED.is_public`, [doc.id, doc.title, doc.title, '/uploads/' + doc.id, doc.body.length, 'text/plain']);
                // Insert content into search_index
                await pool.query(`INSERT INTO search_index (document_id, content, page_number)
           VALUES ($1, $2, 1)
           ON CONFLICT (document_id) DO UPDATE SET
           content = EXCLUDED.content`, [doc.id, doc.body]);
                inserted++;
            }
            catch (docErr) {
